...
const PersonnelStep = ({ 
  ...
  // Check for existing open shift AND fetch data when stationId is available
  useEffect(() => {
    if (stationId) {
  ...
  // Check for existing open shift - This is the KEY function
  const checkExistingOpenShift = async () => {
    ...
    try {
      const openShift = await shiftService.getOpenShift(stationId);
      console.log('📋 Open shift check result:', openShift);
      ...
      if (openShift) {
        ...
        // Update parent component with shift info
        onUpdateShiftInfo({
          shiftId: openShift.id,
          shiftNumber: openShift.shiftNumber,
          supervisorId: openShift.supervisor?.id
        });
        
        // If shift has existing attendants/assignments, load them
        if (openShift.attendants || openShift.assignments) {
          loadExistingShiftData(openShift);
        }
      } else {
  ...
  // Load existing shift data if available
  const loadExistingShiftData = (shiftData) => {
    console.log('📥 Loading existing shift data:', shiftData);
    
    // Load attendants if they exist
    if (shiftData.attendants && Array.isArray(shiftData.attendants)) {
      onUpdatePersonnel({ 
        attendants: shiftData.attendants,
        islandAssignments: shiftData.assignments || []
      });
    }
  };
  ...
  const handleCreateShift = async () => {
    ...
    try {
      ...
      // Create the shift
      const newShift = await onCreateShift(shiftPayload);
      
      // If shift creation was successful, check for the open shift
      if (newShift) {
        await checkExistingOpenShift();
      }
    ...
    if (existingAssignmentIndex >= 0) {
      ...
      newAssignments = [
        ...
        { 
          ...
          shiftId: openShiftData?.id || shiftInfo.shiftId // Link to current shift
        }
      ];
    }
  ...
  const attendantColumns = [
    {
      ...
      render: (id, record) => (
        <input
          ...
          disabled={!hasOpenShift && !shiftInfo.shiftId}
        />
      ),
      width: 60
    },
        ...
        return (
          <Space wrap>
            ...
            <Button 
              ...
              disabled={!personnelData.attendants.some(a => a.id === record.id) || (!hasOpenShift && !shiftInfo.shiftId) || allIslands.length === 0}
            >
              Assign Islands
            </Button>
          </Space>
  ...
  const canProceed = hasOpenShift || !!shiftInfo.shiftId;
  ...
  // Get current shift ID - either from open shift or newly created shift
  const currentShiftId = openShiftData?.id || shiftInfo.shiftId;
  const currentShiftNumber = openShiftData?.shiftNumber || shiftInfo.shiftNumber;
      ...
      {showDebugInfo && (
        <Card 
          title={
            <Space>
              <Database size={16} />
              <Title level={5} style={{ margin: 0 }}>Shift & Mapping Debug</Title>
              <Button 
                size="small" 
          ...
          <Collapse defaultActiveKey={['1', '2', '3']}>
            ...
            <Panel 
              header={
                <Space>
                  <Shield size={14} />
                  <Text strong>Current Shift Information</Text>
                </Space>
              } 
              key="1"
            >
              <Row gutter={[16, 16]}>
                <Col span={6}>
                  <Statistic
                    title="Shift ID"
                    value={currentShiftId ? `${currentShiftId.substring(0, 8)}...` : 'None'}
                    valueStyle={{ color: currentShiftId ? '#3f8600' : '#cf1322' }}
                  />
                </Col>
                <Col span={6}>
                  <Statistic
                    title="Shift Number"
                    value={currentShiftNumber || 'None'}
                    valueStyle={{ color: currentShiftNumber ? '#3f8600' : '#cf1322' }}
                  />
                </Col>
                <Col span={6}>
                  <Statistic
                    title="Has Open Shift"
                    value={hasOpenShift ? 'Yes' : 'No'}
                    valueStyle={{ color: hasOpenShift ? '#3f8600' : '#cf1322' }}
                  />
                </Col>
                <Col span={6}>
                  <Statistic
                    ...
                    valueStyle={{ color: canProceed ? '#3f8600' : '#cf1322' }}
                  />
                </Col>
              </Row>
            </Panel>
            ...
            <Panel 
              header={
                <Space>
                  <Users size={14} />
                  <Text strong>Personnel Summary</Text>
                </Space>
              } 
              key="3"
            >
              <Row gutter={[16, 16]}>
                ...
                <Col span={6}>
                  <Statistic
                    title="Selected Attendants"
                    value={personnelData.attendants.length}
                    valueStyle={{ color: personnelData.attendants.length > 0 ? '#3f8600' : '#cf1322' }}
                  />
                </Col>
                <Col span={6}>
                  <Statistic
                    title="Island Assignments"
                    value={personnelData.islandAssignments.length}
                    valueStyle={{ color: personnelData.islandAssignments.length > 0 ? '#3f8600' : '#cf1322' }}
                  />
                </Col>
              </Row>
            </Panel>
      ...
      <Row gutter={[24, 24]}>
        ...
        <Col span={24}>
          ...
          >
            {hasOpenShift ? (
              <Alert
                message={`Open Shift Found: ${openShiftData.shiftNumber}`}
                description={
                  <Space direction="vertical">
                    <div>
                      <strong>Shift #{openShiftData.shiftNumber}</strong> is currently open.
                      <div style={{ marginTop: 4, fontSize: '12px', color: '#666' }}>
                        ID: {openShiftData.id.substring(0, 12)}...
                      </div>
                    </div>
                    <div>
                    ...
                    <div style={{ marginTop: 8 }}>
                      ...
                      <Tag color="blue">Using Shift ID: {openShiftData.id.substring(0, 8)}...</Tag>
                    </div>
                  </Space>
                }
                type="success"
              ...
              <Row gutter={[16, 16]} align="middle">
                <Col flex="auto">
                  <Space direction="vertical" size="small">
                    ...
                    {currentShiftId && !hasOpenShift && (
                      <Alert
                        message="Shift Created"
                        description={`Shift ${shiftInfo.shiftNumber} was created. You can now assign personnel.`}
                        type="info"
                        showIcon
                        style={{ marginTop: 8 }}
                      />
                ...
                <Col>
                  <Button 
                    ...
                    disabled={loading || fetchingTopology || hasOpenShift}
                    style={{ 
                      background: hasOpenShift ? '#52c41a' : '#1890ff',
                      borderColor: hasOpenShift ? '#52c41a' : '#1890ff',
                      height: '48px',
                      padding: '0 24px',
                      fontWeight: 'bold'
                    }}
                  >
                    {hasOpenShift ? 'Shift Active' : 
            ...
            <Card size="small" type="inner">
              <Row gutter={[16, 16]}>
                ...
                <Col span={8}>
                  <Statistic
                    title="Shift Number"
                    value={currentShiftNumber || 'Loading...'}
          ...
          <Card 
            title={
              <Space>
                ...
                {!canProceed && (
                  <Tag color="orange">No active shift</Tag>
                )}
              </Space>
            } 
            size="small"
            ...
            <Space style={{ marginLeft: 8 }} wrap>
              <Tag color={canProceed ? "green" : "orange"}>
                {hasOpenShift ? "Using Existing Open Shift" : 
                 currentShiftId ? "New Shift Created" : "No Active Shift"}
              </Tag>
              <Tag color={currentShiftId ? "blue" : "orange"}>
                Shift ID: {currentShiftId ? `${currentShiftId.substring(0, 8)}...` : "None"}
              </Tag>
        ...
        {allIslands.length === 0 ? (
          ...
          <Row gutter={[16, 16]}>
            {allIslands.map(island => {
              ...
              const pumpCount = island.pumps?.length || island.directPumps?.length || 0;
              const isSelectable = !!actualIslandId && currentAttendant?.id && canProceed;
                            ...
                            <Tooltip title="Actual Island ID">
                              <Space size={4}>
                                <Text type="secondary" style={{ fontSize: 10 }}>
                                  ID: {actualIslandId.substring(0, 8)}...
                                </Text>
                              </Space>
                            </Tooltip>
                          )}
                          <Tooltip title="Linked Shift ID">
                            <Space size={4}>
                              <Text type="secondary" style={{ fontSize: 10 }}>
                                Shift: {currentShiftId ? currentShiftId.substring(0, 8) + '...' : 'None'}
                              </Text>
                            </Space>
                          </Tooltip>
                        </Space>
                      ...
                      {!isSelectable && (
                        <div style={{ marginTop: 8 }}>
                          <Tag color="red" style={{ fontSize: 10 }}>
                            {!actualIslandId ? "No Asset Mapping" : 
                             !canProceed ? "No Active Shift" : "No Attendant Selected"}
                          </Tag>
                        </div>
                      )}
                    </div>
