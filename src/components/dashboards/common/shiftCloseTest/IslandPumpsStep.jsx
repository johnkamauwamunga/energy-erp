...
const IslandPumpsStep = ({ stationId, closingData, onChange }) => {
  const { state } = useApp();
  ...
  console.log("pumps meter readings ",pumpReadings);
    console.log("attedants  ",attendantsWithDetails);
    console.log("updated collections ",expectedCollectionsByIsland)
    ...
    pumpsWithIslandInfo.forEach(pump => {
      ...
      if (!grouped[islandId]) {
        grouped[islandId] = {
          ...
          attendants: attendantsWithDetails.filter(att => att.islandId === islandId),
          pumps: []
        };
      }
      ...
      grouped[islandId].pumps.push({
        ...
        expectedData: pumpExpectedData // Add expected collection data
      });
    });
    
    return grouped;
  }, [pumpsWithIslandInfo, attendantsWithDetails, pumpReadings, expectedCollectionsByIsland]);
    ...
    Object.values(pumpsByIsland).forEach(island => {
      ...
      island.pumps.forEach(pump => {
        ...
        if (startReading && closingReading && closingReading.electricMeter > 0) {
          ...
          const unitPrice = closingReading.unitPrice || 100.00; // KSH 100.00
    ...
    if (existingReadingIndex >= 0) {
      ...
      updatedReadings = [
        ...pumpReadings,
        {
          ...
          unitPrice: 100.00, // KSH 100.00 as default
          litersDispensed: 0,
          salesValue: 0
        }
      ];
    ...
    return (
      <Alert variant="info" className="text-sm">
        <div className="flex items-start gap-2">
          ...
          <div>
            <p className="font-medium">Loading Pump Data</p>
            <p>Please wait while we fetch the latest pump readings...</p>
          </div>
        </div>
      </Alert>
    );
    ...
    return (
      <Alert variant="warning" className="text-sm">
        <div className="flex items-start gap-2">
          <Zap className="w-4 h-4 mt-0.5 flex-shrink-0" />
          <div>
            <p className="font-medium">Error Loading Pump Data</p>
            <p>{error}</p>
          </div>
        </div>
      </Alert>
