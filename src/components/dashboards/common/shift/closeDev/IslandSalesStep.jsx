...
const IslandSalesStep = ({
  ...
  // Initialize data from readings AND restore cached data
  useEffect(() => {
    if (readingsData?.islands) {
      ...
      readingsData.islands.forEach((island, islandIndex) => {
        ...
        if (preservedSales[islandIndex]) {
          initialEntries[islandIndex] = preservedSales[islandIndex];
        } else {
        ...
        if (preservedReceipts[islandIndex] !== undefined) {
          initialReceipts[islandIndex] = preservedReceipts[islandIndex];
        } else {
          initialReceipts[islandIndex] = 0;
        }
        
        if (preservedExpenses[islandIndex] !== undefined) {
          initialExpenses[islandIndex] = preservedExpenses[islandIndex];
        } else {
          initialExpenses[islandIndex] = 0;
        }
        
        if (preservedCollections[islandIndex]) {
          initialCollections[islandIndex] = preservedCollections[islandIndex];
        } else {
      ...
      const hasCachedData = Object.keys(preservedSales).length > 0 || 
                           Object.keys(preservedCollections).length > 0;
      if (hasCachedData) {
        message.info('Restored cached island sales data');
      }
    }
  }, [readingsData]);
  ...
  useEffect(() => {
    if (onDataPreserve) {
      onDataPreserve(salesEntries, collections, expenses, receipts);
    }
  }, [salesEntries, collections, expenses, receipts, onDataPreserve]);
  ...
  const loadIslandExpenses = async (islands) => {
    ...
    try {
      ...
      Object.entries(expensesData).forEach(([index, data]) => {
        // Only update if we don't have cached data for this island
        if (updatedExpenses[index] === 0 || updatedExpenses[index] === undefined) {
          ...
          <Space direction="vertical" size={4} style={{ width: '100%' }}>
            ...
            <div style={{ 
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <Text style={{ fontSize: '10px', color: '#666' }}>Expected:</Text>
  ...
  const openSummaryModal = () => {
    ...
    // Pass data to parent for caching
    if (onProceedToSummary) {
      onProceedToSummary(finalData);
    }
  };
  ...
  return (
    <div style={{ padding: '16px' }}>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        {/* Show cached data notification */}
        {Object.keys(preservedSales).length > 0 && (
          <Alert
            message="📂 Cached Data Loaded"
            description={`Restored data for ${Object.keys(preservedSales).length} islands from previous session`}
            type="info"
            showIcon
