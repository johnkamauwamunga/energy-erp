...
const ReadingsStep = ({ 
  ...
  const [globalMeterType, setGlobalMeterType] = useState(initialMeterType);

  // Load opening readings from open shift endpoint AND restore cached data
  useEffect(() => {
    if (currentStationId) {
      loadOpenShiftData();
    }
  ...
  // Restore cached data when initial props change
  useEffect(() => {
    if (initialPumps.length > 0 || initialTanks.length > 0) {
      console.log('🔄 Restoring cached data:', {
        pumps: initialPumps.length,
        tanks: initialTanks.length,
        meterType: initialMeterType
      });
      
      setPumps(initialPumps);
      setTanks(initialTanks);
      setGlobalMeterType(initialMeterType);
      setAutoFilled(true);
      setHasCachedData(true);
    }
  }, [initialPumps, initialTanks, initialMeterType]);
  ...
  const loadOpenShiftData = async () => {
    ...
    try {
      ...
      // Only transform pump readings if we don't have cached data
      if (!hasCachedData || pumps.length === 0) {
        ...
        // Only transform tank readings if we don't have cached data
  ...
  const recalculateOtherMeters = useCallback((pumpId, selectedMeterType, enteredValue) => {
    ...
    setPumps(prevPumps => {
      const updatedPumps = prevPumps.map(pump => {
      ...
      // Notify parent of data changes for caching
      if (onDataPreserve) {
        onDataPreserve(updatedPumps, tanks, globalMeterType);
      }
      
      return updatedPumps;
    });
  }, [tanks, globalMeterType, onDataPreserve]);
  ...
  const handleGlobalMeterTypeChange = (newMeterType) => {
    ...
    // Notify parent of data changes for caching
    if (onDataPreserve) {
      onDataPreserve(pumps, tanks, newMeterType);
    }
  };
  ...
  const handlePumpReadingChange = (pumpId, field, value) => {
    setPumps(prev => {
      const updatedPumps = prev.map(pump => 
  ...
  const handleTankReadingChange = (tankId, field, value) => {
    setTanks(prev => {
      ...
      // Notify parent of data changes for caching
      if (onDataPreserve) {
        onDataPreserve(pumps, updatedTanks, globalMeterType);
      }
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        {hasCachedData && (
          <Alert
            ...
            description={`Restored ${pumps.length} pumps and ${tanks.length} tanks from previous session`}
            type="info"
            showIcon
