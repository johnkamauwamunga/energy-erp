...
const CloseWizard = ({ 
  ...
  // State preservation for going back - NOW WITH COMPLETE DATA
  ...
  const [preservedIslandSales, setPreservedIslandSales] = useState({});
  const [preservedCollections, setPreservedCollections] = useState({});
  const [preservedExpenses, setPreservedExpenses] = useState({});
  const [preservedReceipts, setPreservedReceipts] = useState({});
  ...
  const saveDraft = useCallback(() => {
    if (!currentCacheKey) {
      message.warning('Cannot save draft: No cache key available');
      return false;
    }
    ...
    currentCacheKey, preservedPumps, preservedTanks, preservedMeterType,
    currentStep, shift?.id, stationId, readingsData, summaryData,
    preservedIslandSales, preservedCollections, preservedExpenses, preservedReceipts
  ]);
  ...
  const clearAllCache = useCallback(() => {
    ...
    setPreservedIslandSales({});
    setPreservedCollections({});
    setPreservedExpenses({});
    setPreservedReceipts({});
      ...
      if (savedDraft) {
        try {
          ...
          if (draft.shiftId === shift.id && currentTime - draft.timestamp < TWO_HOURS) {
            ...
            setPreservedIslandSales(draft.islandSales || {});
            setPreservedCollections(draft.collections || {});
            setPreservedExpenses(draft.expenses || {});
            setPreservedReceipts(draft.receipts || {});
  ...
  const handleProceedToIslandSales = useCallback((data) => {
    ...
    // Extract and preserve pump readings from the structured data
    const pumps = data.islands?.flatMap(island => 
    ...
    const tanks = data.tankReadings?.map(tank => ({
      id: tank.tankId,
      tankId: tank.tankId,
  ...
  const handleProceedToSummary = useCallback((data) => {
    ...
    // Extract and preserve island sales data
    if (data.islands) {
      const islandSales = {};
      const collections = {};
      const expenses = {};
      ...
      data.islands.forEach((island, index) => {
        islandSales[index] = {
          islandTotalSales: island.totalActualSales || 0,
          notes: ''
        };
        
        // Preserve collections if they exist
        if (island.collections) {
          collections[index] = island.collections;
        }
        
        // Preserve expenses if they exist
        if (island.expenses) {
          expenses[index] = island.expenses;
        }
        
        // Preserve receipts if they exist
        if (island.receipts) {
          receipts[index] = island.receipts;
        }
      });
      
      setPreservedIslandSales(islandSales);
      setPreservedCollections(collections);
      setPreservedExpenses(expenses);
      setPreservedReceipts(receipts);
    }
  ...
  const handleFinalClose = useCallback(async (finalData) => {
    ...
    readingsData, currentUser?.id, shift?.id, stationId, 
    preservedPumps, preservedTanks, preservedMeterType,
    preservedIslandSales, preservedCollections, preservedExpenses, 
    preservedReceipts, onSuccess, clearAllCache
  ]);
  ...
  }, [hasPreservedData, saveDraft, clearAllCache, onClose]);
  ...
  const showCacheStatus = () => {
    ...
    console.log('Preserved Island Sales:', Object.keys(preservedIslandSales).length);
      ...
      case 1: // Island Sales & Collections
        return (
          <IslandSalesStep
            ...
            // Pass ALL preserved data so island sales can restore everything
            preservedSales={preservedIslandSales}
            preservedCollections={preservedCollections}
            preservedExpenses={preservedExpenses}
            preservedReceipts={preservedReceipts}
            // Callback to update preserved data
            onDataPreserve={(sales, collections, expenses, receipts) => {
              console.log('💾 Updating preserved island data');
              setPreservedIslandSales(sales || {});
              setPreservedCollections(collections || {});
              setPreservedExpenses(expenses || {});
              setPreservedReceipts(receipts || {});
              setHasPreservedData(true);
